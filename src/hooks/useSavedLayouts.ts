...
export function useSavedLayouts() {
  ...
  const save = useCallback((title: string): { success: boolean; error?: string } => {
    const trimmedTitle = title.trim()

    // Validate name not taken
    if (isNameTaken(trimmedTitle)) {
      return { success: false, error: 'A layout with this name already exists' }
    }

    // Check if config already saved
    const existingConfig = layouts.find(l => l.url === currentUrl)
    if (existingConfig) {
      return { success: false, error: `This configuration is already saved as "${existingConfig.title}"` }
    }
  ...
  const update = useCallback((id: string): { success: boolean; error?: string } => {
    ...
    const updatedLayouts = layouts.map(l =>
      l.id === id
        ? { ...l, url: currentUrl, updatedAt: Date.now() }
        : l
    )
  ...
  // Rename a layout
  const rename = useCallback((id: string, newTitle: string): { success: boolean; error?: string } => {
    ...
    const updatedLayouts = layouts.map(l =>
      l.id === id
        ? { ...l, title: trimmedTitle, updatedAt: Date.now() }
        : l
    )
    syncToStorage(updatedLayouts)
    return { success: true }
